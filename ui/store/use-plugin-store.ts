...
import { createPdfFromImages, type PdfOptions } from '../lib/pdf'
...
export const usePluginStore = create<PluginState>((set, get) => ({
  ...
  handleExportComplete: async (images) => {
    try {
      const state = get()
      set({ message: `Creating ${state.exportFormat.toUpperCase()} file...` })

      const metadata: PptxMetadata = {
        title: state.filename,
        subject: state.filename,
      }
      const settings: PptxOptions & PdfOptions =
      ...
      const blob =
        state.exportFormat === 'pdf'
          ? await createPdfFromImages(images, settings)
          : await createPptxFromImages(images, settings)
      downloadFile({ filename: `${state.filename}.${state.exportFormat}`, blob })
    } catch (error) {
      set({
